 ...
 * Birden fazla ProjectCopper destekler; her kaydın tüm barlarını gösterir.
 *
 * 4 görünüm: ÖN · YAN · ÜST · ARKA
 */

import type { Panel, ProjectCopper, ProjectPanel } from "../../types";
import {
  buildCabinetLayouts,
  PHASE_COLORS,
  PHASE_LABELS,
  projectCopperToBarRows,
  type BarRow,
} from "./viewHelpers";
...
// ── Ana bileşen ───────────────────────────────────────────────────────────────
export function CopperOrthographicPreview({ panel, projectPanels, projectCoppers = [] }: Props) {
  const { layouts, maxHeight, maxDepth } = buildCabinetLayouts(projectPanels, panel ?? null);
  ...
  // Tüm project copper'lardan barları topla
  const allBarRows: BarRow[] = projectCoppers.flatMap((pc, idx) =>
    projectCopperToBarRows(pc, idx),
  );
  const hasBars = allBarRows.length > 0;
  ...
  const s_vy  = s  ? S2_VY + (VIEW_H - (s?.bh ?? 0)) / 2 : S2_VY + VIEW_H / 2;
  const t_vy  = t  ? S3_VY + (VIEW_H - (t?.bh ?? 0)) / 2 : S3_VY + VIEW_H / 2;
  const bk_vy = S4_VY + (VIEW_H - bk.bh) / 2;

  // Lejant için benzersiz fazlar
  const phaseSet = Array.from(new Set(allBarRows.map((r) => r.phase)));

  // ── Kabin çizimi ────────────────────────────────────────────────────────────
  function drawCabinets(sc: number, ox: number, vy: number, mirror = false): React.ReactNode[] {
  ...
  // ── ÖN / ARKA — XY projeksiyonu ─────────────────────────────────────────────
  function drawBarsXY(sc: number, ox: number, vy: number, mirror = false): React.ReactNode[] {
    return allBarRows.map((row, i) => {
      const phaseIdx = PHASE_LABELS.indexOf(row.phase);
      const color    = PHASE_COLORS[phaseIdx >= 0 ? phaseIdx : 0];
      const barW     = row.width_mm;
  ...
  // ── YAN — ZY projeksiyonu ────────────────────────────────────────────────────
  function drawBarsZY(sc: number, ox: number, vy: number): React.ReactNode[] {
    return allBarRows.map((row, i) => {
      const phaseIdx = PHASE_LABELS.indexOf(row.phase);
      const color    = PHASE_COLORS[phaseIdx >= 0 ? phaseIdx : 0];
      const barW     = row.width_mm;
      const barT     = row.thickness_mm;
  ...
  // ── ÜST — XZ projeksiyonu ────────────────────────────────────────────────────
  function drawBarsXZ(sc: number, ox: number, vy: number): React.ReactNode[] {
    return allBarRows.map((row, i) => {
      const phaseIdx = PHASE_LABELS.indexOf(row.phase);
      const color    = PHASE_COLORS[phaseIdx >= 0 ? phaseIdx : 0];
      const barT     = row.thickness_mm;
  ...
  return (
    ...
    >
      ...
      <text x={VW/2} y={S1_IY+INFO_H-2} textAnchor="middle" fill={C_DIM} fontSize={10} fontFamily="monospace">
        ...
        {hasBars ? `  ·  ${allBarRows.length} bara  (${projectCoppers.length} kayıt)` : ""}
      </text>
      ...
      {hasDepth && s ? (
        <>
          ...
          <line x1={s.ox} y1={s_vy} x2={s.ox} y2={s_vy + deepestLayout.cH * s.sc} stroke={C_OUT} strokeWidth={2.5} />
          ...
          {layouts.map((cl, idx) => {
            ...
            return (
              <g key={`tv-cab-${idx}`}>
                ...
                {idx < layouts.length - 1 && (
                  <line x1={bx+bw} y1={t_vy} x2={bx+bw} y2={t_vy + MD * t!.sc} stroke={C_SEP} strokeWidth={0.5} strokeDasharray="3 3" />
                )}
                {(cl.lm > 0 || cl.rm > 0) && (
      ...
      <g transform={`translate(${PAD},${LEG_Y})`}>
        ...
        {phaseSet.map((phase, idx) => {
          ...
          const color = PHASE_COLORS[pi >= 0 ? pi : 0];
